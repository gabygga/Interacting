...
plt.semilogx(1/(1+backgroundQ['z']), p_scfQ/rho_scfQ, color='b', label='$\Omega_{\phi}$')
#plt.semilogx(1/(1+backgroundQ['z']), (backgroundQ['(.)rho_g']+backgroundQ['(.)rho_ncdm[0]'])/baCritQ, color='c', label='$\Omega_{g}$')
#plt.semilogx(1/(1+backgroundQ['z']), (backgroundQ['(.)rho_b']+backgroundQ['(.)rho_cdm'])/baCritQ, color='g', label='$\Omega_{m}$')
...
plt.ylabel(r"$w_{\phi}$")
plt.tight_layout()
plt.legend(title='$\lambda=8, \\alpha=2, A=0.01, B=34.8, \phi_{ini}=2.2, \dot{\phi}_{ini}=0 $')

# In[ ]:


plt.savefig('w_albrecht_lambda.pdf')
