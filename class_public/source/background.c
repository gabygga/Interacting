...
int background_functions(
  ...
  /* scale factor */
  double a;
  /* scalar field dark matter 1 quantities */
  /*Polar variables arXiv: 1511.08195*/
  double theta_sfdm_1, y1_sfdm_1, alpha_sfdm_1;
  /* scalar field dark matter 2 quantities */
  /*Polar variables arXiv: 1511.08195*/
  double theta_sfdm_2, y1_sfdm_2, alpha_sfdm_2;
  ...
  /*Two Scalar Field Dark Matter*/
  if((pba->has_sfdm_1 == _TRUE_) && (pba->has_sfdm_2 == _TRUE_)){
    /*SFDM1 auxiliaries*/
    theta_sfdm_1 = pvecback_B[pba->index_bi_theta_sfdm_1];
    y1_sfdm_1 = pvecback_B[pba->index_bi_y1_sfdm_1];
    alpha_sfdm_1 = pvecback_B[pba->index_bi_alpha_sfdm_1];
    /*SFDM2 auxiliaries*/
    theta_sfdm_2 = pvecback_B[pba->index_bi_theta_sfdm_2];
    y1_sfdm_2 = pvecback_B[pba->index_bi_y1_sfdm_2];
    alpha_sfdm_2 = pvecback_B[pba->index_bi_alpha_sfdm_2];
    /*SFDM1 quantities*/
    pvecback[pba->index_bg_theta_sfdm_1] = theta_sfdm_1; // value of the angular variable
    pvecback[pba->index_bg_y1_sfdm_1] = y1_sfdm_1; // value of the potential variable
  ...
  pvecback[pba->index_bg_w_tot_1] = p_tot/rho_tot;
  pvecback[pba->index_bg_w_tot_2] = p_tot/rho_tot;
  ...
  if (return_format == pba->long_info) {
    ...
    pvecback[pba->index_bg_Omega_m] = rho_m / pvecback[pba->index_bg_rho_crit];
...
int background_indices(
  ...
  class_define_index(pba->index_bg_w_tot_1,_TRUE_,index_bg,1);
  class_define_index(pba->index_bg_w_tot_2,_TRUE_,index_bg,1);
  ...
  class_define_index(pba->index_bg_rho_dr,pba->has_dr,index_bg,1);
    ...
    /* - indices for scalar field dark matter 1*/
    ...
    /* - indices for scalar field dark matter 2*/
    class_define_index(pba->index_bg_theta_sfdm_2,pba->has_sfdm_2,index_bg,1);
    class_define_index(pba->index_bg_y1_sfdm_2,pba->has_sfdm_2,index_bg,1);
    class_define_index(pba->index_bg_alpha_sfdm_2,pba->has_sfdm_2,index_bg,1);
    class_define_index(pba->index_bg_rho_sfdm_2,pba->has_sfdm_2,index_bg,1);
    class_define_index(pba->index_bg_p_sfdm_2,pba->has_sfdm_2,index_bg,1);
  ...
  class_define_index(pba->index_bi_rho_fld,pba->has_fld,index_bi,1);
    ...
    /* -> scalar field dark matter 2 quantities */
    class_define_index(pba->index_bi_theta_sfdm_2,pba->has_sfdm_2,index_bi,1);
    class_define_index(pba->index_bi_y1_sfdm_2,pba->has_sfdm_2,index_bi,1);
    class_define_index(pba->index_bi_alpha_sfdm_2,pba->has_sfdm_2,index_bi,1);
...
int background_solve(
  ...
  /* initial displacement for axion-SFDM 1 */
  /*double ini_displacement_1=0.;*/
  /* initial displacement for axion-SFDM 2 */
  /*double ini_displacement_2=0.;*/
  ...
  //if (pba->background_verbose > 0) {
    ...
    /*Scalar Field Dark Matter 1*/
    if (pba->has_sfdm_1 == _TRUE_){
        printf("Scalar Field Dark Matter 1 details:\n");
        printf(" -> Omega_sfdm_1 = %g, wished %g\n",
                 exp(pvecback[pba->index_bg_alpha_sfdm_1]), pba->Omega0_sfdm_1);
        printf(" -> lambda_sfdm_1 = %1.2e\n", pba->sfdm_parameters_1[1]);
        /*Print quadratic and cos mass information*/
        if (pba->sfdm_parameters_1[1] >= 0.){
        printf(" -> Mass_sfdm_1 = %1.2e [eV], %1.2e [1/Mpc], %1.2e [H_0]\n",
                    3.19696e-30*pvecback[pba->index_bg_y1_sfdm_1]*pvecback[pba->index_bg_H], 0.5*pvecback[pba->index_bg_y1_sfdm_1]*pvecback[pba->index_bg_H], 0.5*pvecback[pba->index_bg_y1_sfdm_1]);
        }
        /*Print cosh mass information*/
        else{
          printf(" -> Mass_sfdm_1 = %1.2e [eV], %1.2e [1/Mpc], %1.2e [H_0]\n",
                      3.19696e-30*exp(pvecback[pba->index_bg_y1_sfdm_1])*pvecback[pba->index_bg_H], 0.5*exp(pvecback[pba->index_bg_y1_sfdm_1])*pvecback[pba->index_bg_H], 0.5*exp(pvecback[pba->index_bg_y1_sfdm_1]));
        }
        printf("    -> wished = %1.2e [eV]\n", pow(10.,pba->sfdm_parameters_1[0]));
      }
      /*Scalar Field Dark Matter 2*/
      if (pba->has_sfdm_2 == _TRUE_){
        printf("Scalar Field Dark Matter 2 details:\n");
        printf(" -> Omega_sfdm_2 = %g, wished %g\n",
                 exp(pvecback[pba->index_bg_alpha_sfdm_2]), pba->Omega0_sfdm_2);
        printf(" -> lambda_sfdm_2 = %1.2e\n", pba->sfdm_parameters_2[1]);
        /*Print quadratic and cos mass information*/
        if (pba->sfdm_parameters_2[1] >= 0.){
        printf(" -> Mass_sfdm_2 = %1.2e [eV], %1.2e [1/Mpc], %1.2e [H_0]\n",
                    3.19696e-30*pvecback[pba->index_bg_y1_sfdm_2]*pvecback[pba->index_bg_H], 0.5*pvecback[pba->index_bg_y1_sfdm_2]*pvecback[pba->index_bg_H], 0.5*pvecback[pba->index_bg_y1_sfdm_2]);
        }
        /*Print cosh mass information*/
        else{
          printf(" -> Mass_sfdm_2 = %1.2e [eV], %1.2e [1/Mpc], %1.2e [H_0]\n",
                      3.19696e-30*exp(pvecback[pba->index_bg_y1_sfdm_2])*pvecback[pba->index_bg_H], 0.5*exp(pvecback[pba->index_bg_y1_sfdm_2])*pvecback[pba->index_bg_H], 0.5*exp(pvecback[pba->index_bg_y1_sfdm_2]));
        }
...
int background_initial_conditions(
  ...
  */
  ...
  if (pba->has_fld == _TRUE_){
    ...
    /** - Fix initial values of variables of sfdm 1*/
  if(pba->has_sfdm_1 == _TRUE_){
    ...
    !isfinite(pvecback_integration[pba->index_bi_theta_sfdm_1]),
    pba->error_message,
    "initial theta_sfdm_1 = %e alpha_sfdm_1 = %e -> check initial conditions",
    pvecback_integration[pba->index_bi_theta_sfdm_1],
    pvecback_integration[pba->index_bi_alpha_sfdm_1]);
  }
    /** - Fix initial values of variables of sfdm 2*/
  if(pba->has_sfdm_2 == _TRUE_){
    //sfdm_lambda = pba->sfdm_parameters[0];
    pvecback_integration[pba->index_bi_theta_sfdm_2] = pba->theta_ini_sfdm_2;
    pvecback_integration[pba->index_bi_y1_sfdm_2] = pba->y1_ini_sfdm_2;
    pvecback_integration[pba->index_bi_alpha_sfdm_2] = pba->alpha_ini_sfdm_2;

    class_test(!isfinite(pvecback_integration[pba->index_bi_theta_sfdm_2]) ||
    !isfinite(pvecback_integration[pba->index_bi_theta_sfdm_2]),
    pba->error_message,
    "initial theta_sfdm_2 = %e alpha_sfdm_2 = %e -> check initial conditions",
    pvecback_integration[pba->index_bi_theta_sfdm_2],
    pvecback_integration[pba->index_bi_alpha_sfdm_2]);
  }
  ...
  if(pba->has_scf == _TRUE_){
    ...
    scf_B= -log((3*pow(pba->H0,2)*pow(scf_lambda,2)*pba->Omega0_scf)/(scf_A*pow(scf_lambda,2)+pow(1-sqrt(1-pow(scf_lambda*scf_A,2)),2)))/scf_lambda+(1-sqrt(1-pow(scf_lambda*scf_A,2)))/scf_lambda;
    /**-log((3*pow(pba->H0,2)/pow(_c_,2)*pow(scf_lambda,2)*pba->Omega0_scf)/(scf_A*pow(scf_lambda,2)+pow(1-sqrt(1-pow(scf_lambda*scf_A,2)),2)))/scf_lambda+(1-sqrt(1-pow(scf_lambda*scf_A,2)))/scf_lambda;*/
  ...
  /* Just checking that our initial time indeed is deep enough in the radiation
     dominated regime */
  class_test(fabs(pvecback[pba->index_bg_Omega_r]-1.) > 0.1,
	     pba->error_message,
...
int background_output_titles(struct background * pba,
  ...
  /*Scalar Field Dark Matter 2*/
  class_store_columntitle(titles,"(.)rho_sfdm_2",pba->has_sfdm_2);
  class_store_columntitle(titles,"(.)p_sfdm_2",pba->has_sfdm_2);
  class_store_columntitle(titles,"theta_sfdm_2",pba->has_sfdm_2);
  class_store_columntitle(titles,"y1_sfdm_2",pba->has_sfdm_2);
  class_store_columntitle(titles,"alpha_sfdm_2",pba->has_sfdm_2);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    /*Scalar Field Dark Matter 2*/
    class_store_double(dataptr,pvecback[pba->index_bg_rho_sfdm_2],pba->has_sfdm_2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_sfdm_2],pba->has_sfdm_2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_theta_sfdm_2],pba->has_sfdm_2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_y1_sfdm_2],pba->has_sfdm_2,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_alpha_sfdm_2],pba->has_sfdm_2,storeidx);
...
int background_derivs(
  ...
  /*Differential equations for Scalar Fiel Dark Matter 1*/
  if (pba->has_sfdm_1 == _TRUE_){
    ...
    if (pba->sfdm_parameters_1[1] >= 0.){
      ...
      dy[pba->index_bi_y1_sfdm_1] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
        ...
        exp(0.5*y[pba->index_bi_alpha_sfdm_1])*sin_sfdm(pba,0.5*y[pba->index_bi_theta_sfdm_1]));
    }
    /*Cosh*/
    else{
      dy[pba->index_bi_alpha_sfdm_1] = 3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      ...
      dy[pba->index_bi_theta_sfdm_1] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
        ...
        exp(y[pba->index_bi_y1_sfdm_1])*
        pow(1.-pba->sfdm_parameters_1[1]*exp(y[pba->index_bi_alpha_sfdm_1]-2.*y[pba->index_bi_y1_sfdm_1])
           *(1.+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_1])),0.5));
              //pow(pow(y[pba->index_bi_y_phi_scf],2.)-pba->scf_parameters[0]*exp(y[pba->index_bi_Omega_phi_scf])*(1.+cos_scf(pba,y[pba->index_bi_theta_phi_scf])),0.5));
     //General expression for: axion (lambda >0), quadratic (lambda =0), cosh (lambda < 0)
      dy[pba->index_bi_y1_sfdm_1] = 1.5*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (1.+pvecback[pba->index_bg_w_tot_1]);
    }
  }

  /*Differential equations for Scalar Fiel Dark Matter 2*/
  if (pba->has_sfdm_2 == _TRUE_){
    /** - Scalar field dark matter EoM */
    /*Quadratic and cos*/
    if (pba->sfdm_parameters_2[1] >= 0.){
    dy[pba->index_bi_alpha_sfdm_2] = 3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (pvecback[pba->index_bg_w_tot_2]+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_2]));
    dy[pba->index_bi_theta_sfdm_2] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (-3.*sin_sfdm(pba,y[pba->index_bi_theta_sfdm_2])+y[pba->index_bi_y1_sfdm_2]);
    dy[pba->index_bi_y1_sfdm_2] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (1.5*(1.+pvecback[pba->index_bg_w_tot_2])*y[pba->index_bi_y1_sfdm_2]
      + y2_sfdm(pba, y[pba->index_bi_alpha_sfdm_2], y[pba->index_bi_theta_sfdm_2], y[pba->index_bi_y1_sfdm_2], pba->sfdm_parameters_2[1])*
      exp(0.5*y[pba->index_bi_alpha_sfdm_2])*sin_sfdm(pba,0.5*y[pba->index_bi_theta_sfdm_2]));
    }
    /*Cosh*/
    else{
      dy[pba->index_bi_alpha_sfdm_2] = 3.*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      ...
      dy[pba->index_bi_theta_sfdm_2] = y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
        (-3.*sin_sfdm(pba,y[pba->index_bi_theta_sfdm_2])+
        exp(y[pba->index_bi_y1_sfdm_2])*
        pow(1.-pba->sfdm_parameters_2[1]*exp(y[pba->index_bi_alpha_sfdm_2]-2.*y[pba->index_bi_y1_sfdm_2])
           *(1.+cos_sfdm(pba,y[pba->index_bi_theta_sfdm_2])),0.5));
              //pow(pow(y[pba->index_bi_y_phi_scf],2.)-pba->scf_parameters[0]*exp(y[pba->index_bi_Omega_phi_scf])*(1.+cos_scf(pba,y[pba->index_bi_theta_phi_scf])),0.5));
     //General expression for: axion (lambda >0), quadratic (lambda =0), cosh (lambda < 0)
      dy[pba->index_bi_y1_sfdm_2] = 1.5*y[pba->index_bi_a]*pvecback[pba->index_bg_H]*
      (1.+pvecback[pba->index_bg_w_tot_2]);
    }
...
double cos_sfdm(struct background *pba,
    ...
    return 0.5*(1.-tanh(theta_tol*(theta_sfdm*theta_sfdm-theta_thresh*theta_thresh)))*cos(theta_sfdm);
}
...
double sin_sfdm(struct background *pba,
    ...
    double theta_thresh = 1.e2;
    double theta_tol = 1.;//1.e-2;
    return 0.5*(1.-tanh(theta_tol*(theta_sfdm*theta_sfdm-theta_thresh*theta_thresh)))*sin(theta_sfdm);
}

/*SFDM y2 auxiliar variable*/
double y2_sfdm(struct background *pba,
               double Omega_sfdm,
               double theta_sfdm,
               double y1_sfdm,
               double lambda){
                 return lambda*exp(0.5*Omega_sfdm)*cos_sfdm(pba, 0.5*theta_sfdm);
...
double V_scf(
             struct background *pba,
             double phi) {
  return 1.45161*1.e113*V_e_scf(pba,phi)*V_p_scf(pba,phi);
}

double dV_scf(
              struct background *pba,
	      double phi) {
  return 1.45161*1.e113*(dV_e_scf(pba,phi)*V_p_scf(pba,phi) + V_e_scf(pba,phi)*dV_p_scf(pba,phi));
}
