...
int background_initial_conditions(
  ...
  */
  ...
  if(pba->has_scf == _TRUE_){
    ...
    /**scf_B= -log((pow(pba->H0,2)*pow(scf_lambda,2)*pba->Omega0_scf)/(scf_A*pow(scf_lambda,2)+pow(1-sqrt(1-pow(scf_lambda*scf_A,2)),2)))/scf_lambda+(1-sqrt(1-pow(scf_lambda*scf_A,2)))/scf_lambda;
