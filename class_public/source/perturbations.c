...
int perturb_solve(
  ...
  /* Related to sfdm 1*/
  double alpha_sfdm_1, theta_sfdm_1, y1_sfdm_1, m_sfdm_over_H_1;

  /* Related to sfdm 2*/
  double alpha_sfdm_2, theta_sfdm_2, y1_sfdm_2, m_sfdm_over_H_2;
  ...
  while ((tau_upper - tau_lower)/tau_lower > ppr->tol_tau_approx) {
    ...
    /* also check that sfdm is slowly-rolling at early times */
    /*Scalar Field Dark Matter 1*/
    if (pba->has_sfdm_1 == _TRUE_) {
      alpha_sfdm_1 = ppw->pvecback[pba->index_bg_alpha_sfdm_1];
      theta_sfdm_1 = ppw-> pvecback[pba->index_bg_theta_sfdm_1];
      y1_sfdm_1 = ppw->pvecback[pba->index_bg_y1_sfdm_1];
      if(pba->sfdm_parameters_1[1] >= 0.){
        m_sfdm_over_H_1 = 0.5*pow(pow(y1_sfdm_1,2.)+2.*pba->sfdm_parameters_1[1]*exp(alpha_sfdm_1)*
                          cos_sfdm(pba,0.5*theta_sfdm_1)*cos_sfdm(pba,0.5*theta_sfdm_1),0.5);
      }
      if(pba->sfdm_parameters_1[1] < 0.){
        m_sfdm_over_H_1 = 0.5*exp(ppw->pvecback[pba->index_bg_y1_sfdm_1]);
      }
    ...
    /*Scalar Field Dark Matter 2*/
    if (pba->has_sfdm_2 == _TRUE_) {
      alpha_sfdm_2 = ppw->pvecback[pba->index_bg_alpha_sfdm_2];
      theta_sfdm_2 = ppw-> pvecback[pba->index_bg_theta_sfdm_2];
      y1_sfdm_2 = ppw->pvecback[pba->index_bg_y1_sfdm_2];
      if(pba->sfdm_parameters_2[1] >= 0.){
        m_sfdm_over_H_2 = 0.5*pow(pow(y1_sfdm_2,2.)+2.*pba->sfdm_parameters_2[1]*exp(alpha_sfdm_2)*
                          cos_sfdm(pba,0.5*theta_sfdm_2)*cos_sfdm(pba,0.5*theta_sfdm_2),0.5);
      }
      if(pba->sfdm_parameters_2[1] < 0.){
        m_sfdm_over_H_2 = 0.5*exp(ppw->pvecback[pba->index_bg_y1_sfdm_2]);
      }
      if (m_sfdm_over_H_2 > 1.e-2)
        is_early_enough = _FALSE_;
    }
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* Scalar Field Dark Matter sfdm 1*/
      ...
      //class_store_columntitle(ppt->scalar_titles, "fk_sfdm_1", pba->has_sfdm_1);//Como no necesita ser integrada no se define
      /* Scalar Field Dark Matter sfdm 2*/
      class_store_columntitle(ppt->scalar_titles, "delta_sfdm_2", pba->has_sfdm_2);
      class_store_columntitle(ppt->scalar_titles, "theta_sfdm_2", pba->has_sfdm_2);
      class_store_columntitle(ppt->scalar_titles, "omega_sfdm_2", pba->has_sfdm_2);
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    /* Scalar Field Fark Matter 1*/
    class_define_index(ppv->index_pt_omega_sfdm_1, pba->has_sfdm_1,index_pt,1);/*sfdm_1 frequency*/
    class_define_index(ppv->index_pt_delta_sfdm_1,pba->has_sfdm_1,index_pt,1); /* scalar field dark matter denstiy constrast */
    class_define_index(ppv->index_pt_delta1_sfdm_1,pba->has_sfdm_1,index_pt,1); /* scalar field dark matter auxiliary contrast. See arXiv: 1511.08195*/

    /* Scalar Field Fark Matter 2*/
    class_define_index(ppv->index_pt_omega_sfdm_2, pba->has_sfdm_2,index_pt,1);/*sfdm_2 frequency*/
    class_define_index(ppv->index_pt_delta_sfdm_2,pba->has_sfdm_2,index_pt,1); /* scalar field dark matter denstiy constrast */
    class_define_index(ppv->index_pt_delta1_sfdm_2,pba->has_sfdm_2,index_pt,1); /* scalar field dark matter auxiliary contrast. See arXiv: 1511.08195*/
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  double delta_dr=0;
  /*Scalar Field Dark Matter 1*/
  double delta_sfdm_1=0.,theta_sfdm_1=0.;
  /*Scalar Field Dark Matter 2*/
  double delta_sfdm_2=0.,theta_sfdm_2=0.;
  double q,epsilon,k2;
  int index_q,n_ncdm,idx;
  double rho_r,rho_m,rho_nu,rho_m_over_rho_r;
  double fracnu,fracg,fracb,fraccdm,fracsfdm_1, fracsfdm_2,om;
  ...
  if (_scalars_) {
    ...
    */
    ...
    fraccdm = 1.-fracb-fracsfdm_1-fracsfdm_2;
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      if (pba->has_sfdm_1 == _TRUE_) {
         ...
         *  initial perturbations set to the attractor solution, see arXiv:1511.08195, arXiv:1703.10180, and the frequency of sfdm */
        if (pba->sfdm_parameters_1[1] >= 0.){
          ppw->pv->y[ppw->pv->index_pt_omega_sfdm_1] = k*k/(pow(a*ppw->pvecback[pba->index_bg_H],2.)*ppw->pvecback[pba->index_bg_y1_sfdm_1]);
        }
        if(pba->sfdm_parameters_1[1] < 0.){
          ppw->pv->y[ppw->pv->index_pt_omega_sfdm_1] = k*k/(pow(a*ppw->pvecback[pba->index_bg_H],2.)*
                                                         exp(ppw->pvecback[pba->index_bg_y1_sfdm_1])*
                                                         pow(1.-pba->sfdm_parameters_1[1]*exp(ppw->pvecback[pba->index_bg_alpha_sfdm_1])*(1.+cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1]))/exp(2.*ppw->pvecback[pba->index_bg_y1_sfdm_1]),0.5));
        }
        ppw->pv->y[ppw->pv->index_pt_delta_sfdm_1] = (3./7.)*ppw->pv->y[ppw->pv->index_pt_delta_g]*sin(0.5*ppw->pvecback[pba->index_bg_theta_sfdm_1])*
        sin(ppw->pvecback[pba->index_bg_theta_sfdm_1]/12.);
        ppw->pv->y[ppw->pv->index_pt_delta1_sfdm_1] = (3./7.)*ppw->pv->y[ppw->pv->index_pt_delta_g]*sin(0.5*ppw->pvecback[pba->index_bg_theta_sfdm_1])*
      ...
      if (pba->has_sfdm_2 == _TRUE_) {
         ...
         *  initial perturbations set to the attractor solution, see arXiv:1511.08195, arXiv:1703.10180. */
         if (pba->sfdm_parameters_2[1] >= 0.){
           ppw->pv->y[ppw->pv->index_pt_omega_sfdm_2] = k*k/(pow(a*ppw->pvecback[pba->index_bg_H],2.)*ppw->pvecback[pba->index_bg_y1_sfdm_2]);
         }
         if(pba->sfdm_parameters_2[1] < 0.){
           ppw->pv->y[ppw->pv->index_pt_omega_sfdm_2] = k*k/(pow(a*ppw->pvecback[pba->index_bg_H],2.)*
                                                          exp(ppw->pvecback[pba->index_bg_y1_sfdm_2])*
                                                          pow(1.-pba->sfdm_parameters_2[1]*exp(ppw->pvecback[pba->index_bg_alpha_sfdm_2])*(1.+cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2]))/exp(2.*ppw->pvecback[pba->index_bg_y1_sfdm_2]),0.5));
         }
        ppw->pv->y[ppw->pv->index_pt_delta_sfdm_2] = (3./7.)*ppw->pv->y[ppw->pv->index_pt_delta_g]*sin(0.5*ppw->pvecback[pba->index_bg_theta_sfdm_2])*
        sin(ppw->pvecback[pba->index_bg_theta_sfdm_2]/12.);
        ppw->pv->y[ppw->pv->index_pt_delta1_sfdm_2] = (3./7.)*ppw->pv->y[ppw->pv->index_pt_delta_g]*sin(0.5*ppw->pvecback[pba->index_bg_theta_sfdm_2])*
    ...
    if (ppt->gauge == newtonian) {
      ...
      */
      ...
      /*Scalar Field Dark Matter 1*/
      if (pba->has_sfdm_1 == _TRUE_) {
        delta_sfdm_1 = ppw->pv->y[ppw->pv->index_pt_delta_sfdm_1];
      ...
      /*Scalar Field Dark Matter 2*/
      if (pba->has_sfdm_2 == _TRUE_) {
        delta_sfdm_2 = ppw->pv->y[ppw->pv->index_pt_delta_sfdm_2];
          // TODO: Include the right value of theta_sfdm here.
        theta_sfdm_2 = 0.;
      }
      ...
      delta_tot = (fracg*ppw->pv->y[ppw->pv->index_pt_delta_g]+fracnu*delta_ur+rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_delta_b]+fraccdm*delta_cdm+fracsfdm_1*delta_sfdm_1+fracsfdm_2*delta_sfdm_2))/(1.+rho_m_over_rho_r);

      velocity_tot = ((4./3.)*(fracg*ppw->pv->y[ppw->pv->index_pt_theta_g]+fracnu*theta_ur) + rho_m_over_rho_r*(fracb*ppw->pv->y[ppw->pv->index_pt_theta_b]+fracsfdm_1*theta_sfdm_1+fracsfdm_2*theta_sfdm_2))/(1.+rho_m_over_rho_r);
      ...
      /*Scalar Field Dark Matter 1*/
      if (pba->has_sfdm_1 == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_sfdm_1] -= 3.*a_prime_over_a*alpha*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1]));
        //ppw->pv->y[ppw->pv->index_pt_theta_sfdm] += k*k*alpha;
      }

      /*Scalar Field Dark Matter 2*/
      if (pba->has_sfdm_2 == _TRUE_) {
        ppw->pv->y[ppw->pv->index_pt_delta_sfdm_2] -= 3.*a_prime_over_a*alpha*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2]));
        //ppw->pv->y[ppw->pv->index_pt_theta_sfdm] += k*k*alpha;
      }
...
int perturb_total_stress_energy(
  ...
  /*Scalar Field Dark Matter 2*/
  double delta_rho_sfdm_2, delta_p_sfdm_2, rho_plus_p_theta_sfdm_2;
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      /* scalar field dark matter contribution.
       TODO: Verify expressions for th Newtonian gauge.
       */
       /*Scalar Field Dark Matter 1. See arXiv: 1511.08195*/
      if (pba->has_sfdm_1 == _TRUE_) {

          //if (ppt->gauge == synchronous){
              delta_rho_sfdm_1 = ppw->pvecback[pba->index_bg_rho_sfdm_1]*y[ppw->pv->index_pt_delta_sfdm_1];
              delta_p_sfdm_1 = ppw->pvecback[pba->index_bg_rho_sfdm_1]*(y[ppw->pv->index_pt_delta1_sfdm_1]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])
                                                                    -y[ppw->pv->index_pt_delta_sfdm_1]*cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1]));
              if (pba->sfdm_parameters_1[1] >= 0.){
                  rho_plus_p_theta_sfdm_1 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_1]*(-y[ppw->pv->index_pt_delta_sfdm_1]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])+
                                                                                 y[ppw->pv->index_pt_delta1_sfdm_1]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])))/
                  (a*ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_y1_sfdm_1]);
              }
              if(pba->sfdm_parameters_1[1] < 0.){
                rho_plus_p_theta_sfdm_1 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_1]*(-y[ppw->pv->index_pt_delta_sfdm_1]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])+
                                                                               y[ppw->pv->index_pt_delta1_sfdm_1]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])))/
                (a*ppw->pvecback[pba->index_bg_H]*exp(ppw->pvecback[pba->index_bg_y1_sfdm_1])*pow(1.-pba->sfdm_parameters_1[1]*exp(ppw->pvecback[pba->index_bg_alpha_sfdm_1])*
              (1. + cos_sfdm(pba, ppw->pvecback[pba->index_bg_theta_sfdm_1]))/exp(2.*ppw->pvecback[pba->index_bg_y1_sfdm_1]),0.5));
              }
          //}
          //else{
              /* Unverified! */
              //delta_rho_sfdm = ppw->pvecback[pba->index_bg_rho_sfdm]*y[ppw->pv->index_pt_delta_sfdm];
              //delta_p_sfdm = ppw->pvecback[pba->index_bg_rho_sfdm]*(y[ppw->pv->index_pt_delta1_sfdm]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm])
                                                                    //-y[ppw->pv->index_pt_delta_sfdm]*cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm]));
              //rho_plus_p_theta_sfdm = k*k*ppw->pvecback[pba->index_bg_rho_sfdm]*(-y[ppw->pv->index_pt_delta_sfdm]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm])+
              //                                                                   y[ppw->pv->index_pt_delta1_sfdm]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm])))/
              //(a*ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_y1_sfdm]);
          //}
          ...
          rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_sfdm_1]+ppw->pvecback[pba->index_bg_p_sfdm_1];

      }

      /*Scalar Field Dark Matter 2. See arXiv: 1511.08195*/
     if (pba->has_sfdm_2 == _TRUE_) {

         //if (ppt->gauge == synchronous){
             delta_rho_sfdm_2 = ppw->pvecback[pba->index_bg_rho_sfdm_2]*y[ppw->pv->index_pt_delta_sfdm_2];
             delta_p_sfdm_2 = ppw->pvecback[pba->index_bg_rho_sfdm_2]*(y[ppw->pv->index_pt_delta1_sfdm_2]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])
                                                                   -y[ppw->pv->index_pt_delta_sfdm_2]*cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2]));
            if (pba->sfdm_parameters_2[1] >= 0.){
                rho_plus_p_theta_sfdm_2 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_2]*(-y[ppw->pv->index_pt_delta_sfdm_2]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])+
                                          y[ppw->pv->index_pt_delta1_sfdm_2]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])))/
                                          (a*ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_y1_sfdm_2]);
            }
            if(pba->sfdm_parameters_2[1] < 0.){
                rho_plus_p_theta_sfdm_2 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_2]*(-y[ppw->pv->index_pt_delta_sfdm_2]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])+
                                          y[ppw->pv->index_pt_delta1_sfdm_2]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])))/
                                          (a*ppw->pvecback[pba->index_bg_H]*exp(ppw->pvecback[pba->index_bg_y1_sfdm_2])*pow(1.-pba->sfdm_parameters_2[1]*exp(ppw->pvecback[pba->index_bg_alpha_sfdm_2])*
                                                                   (1. + cos_sfdm(pba, ppw->pvecback[pba->index_bg_theta_sfdm_2]))/exp(2.*ppw->pvecback[pba->index_bg_y1_sfdm_2]),0.5));
            }
         //else{
             /* Unverified! */
             //delta_rho_sfdm = ppw->pvecback[pba->index_bg_rho_sfdm]*y[ppw->pv->index_pt_delta_sfdm];
         ...
         ppw->delta_rho += delta_rho_sfdm_2;

         ppw->rho_plus_p_theta += rho_plus_p_theta_sfdm_2;

         ppw->delta_p += delta_p_sfdm_2;

         rho_plus_p_tot += ppw->pvecback[pba->index_bg_rho_sfdm_2]+ppw->pvecback[pba->index_bg_p_sfdm_2];
    ...
    */
    ...
    if (ppt->has_source_delta_m == _TRUE_) {
      ...
      /* Include the sfdm to the matter density constrast */
      /*Scalar Field Dark Matter 1*/
      if (pba->has_sfdm_1 == _TRUE_) {
    ...
    if ((ppt->has_source_delta_m == _TRUE_) || (ppt->has_source_theta_m == _TRUE_)) {
     ...
     /* Include the sfdm to the matter velocity divergence */
     /*Scalar Field Dark Matter 1*/
     if (pba->has_sfdm_1 == _TRUE_) {
        rho_plus_p_theta_m += rho_plus_p_theta_sfdm_1;
        rho_plus_p_m += ppw->pvecback[pba->index_bg_rho_sfdm_1]+ppw->pvecback[pba->index_bg_p_sfdm_1];
     }
     /*Scalar Field Dark Matter 2*/
     if (pba->has_sfdm_2 == _TRUE_) {
        rho_plus_p_theta_m += rho_plus_p_theta_sfdm_2;
        rho_plus_p_m += ppw->pvecback[pba->index_bg_rho_sfdm_2]+ppw->pvecback[pba->index_bg_p_sfdm_2];
     }
...
int perturb_sources(
  ...
  /*Scalar Field Dark Matter 1*/
  double rho_plus_p_theta_sfdm_1;
  /*Scalar Field Dark Matter 2*/
  double rho_plus_p_theta_sfdm_2;
  ...
  if (_scalars_) {
    ...
    if (ppt->has_source_delta_fld == _TRUE_) {
      ...
      if (ppt->has_source_delta_sfdm_1 == _TRUE_) {
          //if (ppt->gauge == synchronous){
          //    delta_rho_sfdm = pvecback[pba->index_bg_rho_sfdm]*y[ppw->pv->index_pt_delta_sfdm]);
          //}
          //else{
          //    delta_rho_sfdm = pvecback[pba->index_bg_rho_sfdm]*y[ppw->pv->index_pt_delta_sfdm];
          //}
        _set_source_(ppt->index_tp_delta_sfdm_1) = y[ppw->pv->index_pt_delta_sfdm_1];
      }
      /*Scalar Field Dark Matter 2*/
      if (ppt->has_source_delta_sfdm_2 == _TRUE_) {
          //if (ppt->gauge == synchronous){
          //    delta_rho_sfdm = pvecback[pba->index_bg_rho_sfdm]*y[ppw->pv->index_pt_delta_sfdm]);
          //}
          //else{
          //    delta_rho_sfdm = pvecback[pba->index_bg_rho_sfdm]*y[ppw->pv->index_pt_delta_sfdm];
          //}
        _set_source_(ppt->index_tp_delta_sfdm_2) = y[ppw->pv->index_pt_delta_sfdm_2];
      }
    ...
    if (ppt->has_source_theta_sfdm_1 == _TRUE_) {
      if (pba->sfdm_parameters_1[1] >= 0.){
        rho_plus_p_theta_sfdm_1 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_1]*(-y[ppw->pv->index_pt_delta_sfdm_1]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])+
                                                                               y[ppw->pv->index_pt_delta1_sfdm_1]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])))/
        (a_rel*ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_y1_sfdm_1]);
      }
      if(pba->sfdm_parameters_1[1] < 0.){
        rho_plus_p_theta_sfdm_1 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_1]*(-y[ppw->pv->index_pt_delta_sfdm_1]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])+
                                                                               y[ppw->pv->index_pt_delta1_sfdm_1]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])))/
        (a_rel*ppw->pvecback[pba->index_bg_H]*exp(ppw->pvecback[pba->index_bg_y1_sfdm_1]));
      }

      _set_source_(ppt->index_tp_theta_sfdm_1) = rho_plus_p_theta_sfdm_1/(pvecback[pba->index_bg_rho_sfdm_1]+pvecback[pba->index_bg_p_sfdm_1]);
    }
    /*Scalar Field Dark Matter 2*/
    if (ppt->has_source_theta_sfdm_2 == _TRUE_) {
      if (pba->sfdm_parameters_2[1] >= 0.){
        rho_plus_p_theta_sfdm_2 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_2]*(-y[ppw->pv->index_pt_delta_sfdm_2]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])+
                                                                               y[ppw->pv->index_pt_delta1_sfdm_2]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])))/
        (a_rel*ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_y1_sfdm_2]);
      }
      if(pba->sfdm_parameters_2[1] < 0.){
        rho_plus_p_theta_sfdm_2 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_2]*(-y[ppw->pv->index_pt_delta_sfdm_2]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])+
                                                                               y[ppw->pv->index_pt_delta1_sfdm_2]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])))/
        (a_rel*ppw->pvecback[pba->index_bg_H]*exp(ppw->pvecback[pba->index_bg_y1_sfdm_2]));
      }

      _set_source_(ppt->index_tp_theta_sfdm_2) = rho_plus_p_theta_sfdm_2/(pvecback[pba->index_bg_rho_sfdm_2]+pvecback[pba->index_bg_p_sfdm_2]);
    }
...
int perturb_print_variables(double tau,
  ...
  /*Scalar Field Dark Matter 1*/
  double delta_rho_sfdm_1=0., rho_plus_p_theta_sfdm_1=0., omega_sfdm_1=0.;
  double delta_sfdm_1=0., theta_sfdm_1=0.;
  /*Scalar Field Dark Matter 2*/
  double delta_rho_sfdm_2=0., rho_plus_p_theta_sfdm_2=0., omega_sfdm_2=0.;
  double delta_sfdm_2=0., theta_sfdm_2=0.;
  ...
  if (_scalars_) {
    ...
    if (pba->has_sfdm_1 == _TRUE_){
      ...
      rho_plus_p_theta_sfdm_1 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_1]*(-y[ppw->pv->index_pt_delta_sfdm_1]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])+
                                                                           y[ppw->pv->index_pt_delta1_sfdm_1]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_1])))/
        (a*ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_y1_sfdm_1]);
      theta_sfdm_1 = rho_plus_p_theta_sfdm_1/(pvecback[pba->index_bg_rho_sfdm_1]+pvecback[pba->index_bg_p_sfdm_1]);

    }
    /*Scalar Field Dark Matter 2*/
    if (pba->has_sfdm_2 == _TRUE_){
      omega_sfdm_2 = y[ppw->pv->index_pt_omega_sfdm_2];
      delta_sfdm_2 = y[ppw->pv->index_pt_delta_sfdm_2];
      rho_plus_p_theta_sfdm_2 = k*k*ppw->pvecback[pba->index_bg_rho_sfdm_2]*(-y[ppw->pv->index_pt_delta_sfdm_2]*sin_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])+
                                                                           y[ppw->pv->index_pt_delta1_sfdm_2]*(1.-cos_sfdm(pba,ppw->pvecback[pba->index_bg_theta_sfdm_2])))/
        (a*ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_y1_sfdm_2]);
      theta_sfdm_2 = rho_plus_p_theta_sfdm_2/(pvecback[pba->index_bg_rho_sfdm_2]+pvecback[pba->index_bg_p_sfdm_2]);
    }
    ...
    if (pba->has_int_scf == _TRUE_) {
       if (ppt->gauge == synchronous){
          delta_int = (pba->int_sf)/sqrt(8*_PI_*_G_)*pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*(ppw->pvecback[pba->index_bg_phi_prime_scf]*sqrt(3/2)*exp(0.5*y[pba->index_bi_alpha_sfdm_1])*(y[ppw->pv->index_pt_delta1_sfdm_1]*cos_sfdm(pba,0.5*ppw->pvecback[pba->index_bg_theta_sfdm_1])+y[ppw->pv->index_pt_delta_sfdm_1]*sin_sfdm(pba, 0.5*ppw->pvecback[pba->index_bg_theta_sfdm_1]))+exp(-0.5*y[pba->index_bi_alpha_sfdm_1])*(sqrt(2./3.)*sin_sfdm(pba, 0.5*pvecback[pba->index_bg_theta_sfdm_1])*y[ppw->pv->index_pt_phi_prime_scf]));
       }
    }
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_sfdm_1 == _TRUE_) {
        ...
        delta_sfdm_1 -= 3.0*pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha*(1.0-cos_sfdm(pba,pvecback[pba->index_bg_theta_sfdm_1]));
        theta_sfdm_1 += k*k*alpha;
      }
      /*Scalar Field Dark Matter 2*/
      if (pba->has_sfdm_2 == _TRUE_) {
          /** TODO: verification of the gauge transformation */
        delta_sfdm_2 -= 3.0*pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha*(1.0-cos_sfdm(pba,pvecback[pba->index_bg_theta_sfdm_2]));
        theta_sfdm_2 += k*k*alpha;
      }
    ...
    /* Scalar field dark matter 1 sfdm */
    ...
    /* Scalar field dark matter 2 sfdm */
    class_store_double(dataptr, delta_sfdm_2, pba->has_sfdm_2, storeidx);
    class_store_double(dataptr, theta_sfdm_2, pba->has_sfdm_2, storeidx);
    class_store_double(dataptr, omega_sfdm_2, pba->has_sfdm_2, storeidx);
    /* Scalar field scf*/
...
int perturb_derivs(double tau,
  ...
  /* for use with scalar field dark matter 1 */
  double alpha_sfdm_1,theta_sfdm_1, y1_sfdm_1, omega_sfdm_1, delta_sfdm_1, delta1_sfdm_1;

  /* for use with scalar field dark matter 2 */
  double alpha_sfdm_2,theta_sfdm_2, y1_sfdm_2, omega_sfdm_2, delta_sfdm_2, delta1_sfdm_2;
  ...
  if (_scalars_) {
    ...
    /*Scalar Field Dark Matter 1*/
    if (pba->has_sfdm_1 == _TRUE_) {
      alpha_sfdm_1 = pvecback[pba->index_bg_alpha_sfdm_1];
      theta_sfdm_1 = pvecback[pba->index_bg_theta_sfdm_1];
      y1_sfdm_1 = pvecback[pba->index_bg_y1_sfdm_1];
      ...
      if (pba->sfdm_parameters_1[1] >= 0.){
      /*omega*/
      dy[pv->index_pt_omega_sfdm_1] = a_prime_over_a*omega_sfdm_1*(1.5*pvecback[pba->index_bg_w_tot_1] - 0.5
                                       -y2_sfdm(pba, alpha_sfdm_1, theta_sfdm_1, y1_sfdm_1, pba->sfdm_parameters_1[1])*
                                       exp(0.5*alpha_sfdm_1)*sin_sfdm(pba,0.5*theta_sfdm_1)/y1_sfdm_1);
      /*delta_0*/
      dy[pv->index_pt_delta_sfdm_1] = -a_prime_over_a*((3.*sin_sfdm(pba,theta_sfdm_1)+omega_sfdm_1*(1.-cos_sfdm(pba,theta_sfdm_1)))*delta1_sfdm_1
       ...
       -pba->int_sf*a_prime_over_a*pvecback[pba->index_bg_phi_prime_scf]/a*sin_sfdm(pba, 0.5*theta_sfdm_1)*(pvecback[pba->index_bg_phi_prime_scf]/a*(delta1_sfdm_1*cos_sfdm(pba, 0.5*theta_sfdm_1)+sin_sfdm(pba, 0.5*theta_sfdm_1)*delta_sfdm_1)+4*sin_sfdm(pba, 0.5*theta_sfdm_1)*pv->index_pt_phi_prime_scf/a
                                       -pvecback[pba->index_bg_phi_prime_scf]/a*(-delta1_sfdm_1*cos_sfdm(pba, 0.5*theta_sfdm_1)+sin_sfdm(pba, 0.5*theta_sfdm_1)*delta_sfdm_1));
; //metric_continuity = h'/2;
      ...
      dy[pv->index_pt_delta1_sfdm_1] = -a_prime_over_a*((3.*cos_sfdm(pba,theta_sfdm_1)+(omega_sfdm_1-0.5*pba->sfdm_parameters_1[1]*exp(alpha_sfdm_1)/y1_sfdm_1)*sin_sfdm(pba,theta_sfdm_1))*delta1_sfdm_1
                                       ...
                                       -pba->int_sf*a_prime_over_a*pvecback[pba->index_bg_phi_prime_scf]/a*cos_sfdm(pba, 0.5*theta_sfdm_1)*(pvecback[pba->index_bg_phi_prime_scf]/a*(delta1_sfdm_1*cos_sfdm(pba, 0.5*theta_sfdm_1)+sin_sfdm(pba, 0.5*theta_sfdm_1)*delta_sfdm_1)+4*sin_sfdm(pba, 0.5*theta_sfdm_1)*pv->index_pt_phi_prime_scf/a)
                                       +pba->int_sf*a_prime_over_a*pvecback[pba->index_bg_phi_prime_scf]/a*sin_sfdm(pba, 0.5*theta_sfdm_1)*pvecback[pba->index_bg_phi_prime_scf]/a*(delta1_sfdm_1*cos_sfdm(pba, 0.5*theta_sfdm_1)+sin_sfdm(pba, 0.5*theta_sfdm_1)*delta_sfdm_1);
     }
     if(pba->sfdm_parameters_1[1] < 0.){
       dy[pv->index_pt_omega_sfdm_1] = a_prime_over_a*omega_sfdm_1*(1.5*pvecback[pba->index_bg_w_tot_1] - 0.5 - 0.5*pba->sfdm_parameters_1[1]*
                                       exp(alpha_sfdm_1-y1_sfdm_1)*sin_sfdm(pba, theta_sfdm_1)/pow(1.-pba->sfdm_parameters_1[1]*
                                       exp(alpha_sfdm_1-2.*y1_sfdm_1)*(1.+cos_sfdm(pba, theta_sfdm_1)),0.5));
       /*delta_0*/
      dy[pv->index_pt_delta_sfdm_1] = -a_prime_over_a*((3.*sin_sfdm(pba,theta_sfdm_1)+omega_sfdm_1*(1.-cos_sfdm(pba,theta_sfdm_1)))*delta1_sfdm_1
                                      -omega_sfdm_1*sin_sfdm(pba,theta_sfdm_1)*delta_sfdm_1)
                                     -metric_continuity*(1.-cos_sfdm(pba,theta_sfdm_1)); //metric_continuity = h'/2;
      /*delta_1*/
     dy[pv->index_pt_delta1_sfdm_1] = -a_prime_over_a*((3.*cos_sfdm(pba,theta_sfdm_1)+(omega_sfdm_1-0.5*pba->sfdm_parameters_1[1]*exp(alpha_sfdm_1-y1_sfdm_1)/
                                      pow(1.-pba->sfdm_parameters_1[1]*exp(alpha_sfdm_1-2.*y1_sfdm_1)*(1.+cos_sfdm(pba,theta_sfdm_1)),0.5))*
                                      sin_sfdm(pba,theta_sfdm_1))*delta1_sfdm_1
                                      -(omega_sfdm_1-0.5*pba->sfdm_parameters_1[1]*exp(alpha_sfdm_1-y1_sfdm_1)/
                                       pow(1.-pba->sfdm_parameters_1[1]*exp(alpha_sfdm_1-2.*y1_sfdm_1)*(1.+cos_sfdm(pba, theta_sfdm_1)),0.5))*
                                      (1.+cos_sfdm(pba,theta_sfdm_1))*delta_sfdm_1)
                                      -metric_continuity*sin_sfdm(pba,theta_sfdm_1);
     }
    }
    ...
    /*Scalar Field Dark Matter 2*/
    if (pba->has_sfdm_2 == _TRUE_) {
      alpha_sfdm_2 = pvecback[pba->index_bg_alpha_sfdm_2];
      theta_sfdm_2 = pvecback[pba->index_bg_theta_sfdm_2];
      y1_sfdm_2 = pvecback[pba->index_bg_y1_sfdm_2];
      omega_sfdm_2 = y[pv->index_pt_omega_sfdm_2];
      delta_sfdm_2 = y[pv->index_pt_delta_sfdm_2];
      delta1_sfdm_2 = y[pv->index_pt_delta1_sfdm_2];
      /*Equations of motion*/
      if (pba->sfdm_parameters_2[1] >= 0.){
      /*omega*/
      dy[pv->index_pt_omega_sfdm_2] = a_prime_over_a*omega_sfdm_2*(1.5*pvecback[pba->index_bg_w_tot_2] - 0.5
                                       -y2_sfdm(pba, alpha_sfdm_2, theta_sfdm_2, y1_sfdm_2, pba->sfdm_parameters_2[1])*
                                       exp(0.5*alpha_sfdm_2)*sin_sfdm(pba,0.5*theta_sfdm_2)/y1_sfdm_2);
      /*delta_0*/
      dy[pv->index_pt_delta_sfdm_2] = -a_prime_over_a*((3.*sin_sfdm(pba,theta_sfdm_2)+omega_sfdm_2*(1.-cos_sfdm(pba,theta_sfdm_2)))*delta1_sfdm_2
                                                         -omega_sfdm_2*sin_sfdm(pba,theta_sfdm_2)*delta_sfdm_2)
        -metric_continuity*(1.-cos_sfdm(pba,theta_sfdm_2)); //metric_continuity = h'/2;
      /*delta_1*/
      dy[pv->index_pt_delta1_sfdm_2] = -a_prime_over_a*((3.*cos_sfdm(pba,theta_sfdm_2)+(omega_sfdm_2-0.5*pba->sfdm_parameters_2[1]*exp(alpha_sfdm_2)/y1_sfdm_2)*sin_sfdm(pba,theta_sfdm_2))*delta1_sfdm_2
                                       -((omega_sfdm_2-0.5*pba->sfdm_parameters_2[1]*exp(alpha_sfdm_2)/y1_sfdm_2)*(1.+cos_sfdm(pba,theta_sfdm_2))*delta_sfdm_2))
                                       -metric_continuity*sin_sfdm(pba,theta_sfdm_2);
     }
     if(pba->sfdm_parameters_2[1] < 0.){
       dy[pv->index_pt_omega_sfdm_2] = a_prime_over_a*omega_sfdm_2*(1.5*pvecback[pba->index_bg_w_tot_2] - 0.5 - 0.5*pba->sfdm_parameters_2[1]*
                                       exp(alpha_sfdm_2-y1_sfdm_2)*sin_sfdm(pba, theta_sfdm_2)/pow(1.-pba->sfdm_parameters_2[1]*
                                       exp(alpha_sfdm_2-2.*y1_sfdm_2)*(1.+cos_sfdm(pba, theta_sfdm_2)),0.5));
       /*delta_0*/
      dy[pv->index_pt_delta_sfdm_2] = -a_prime_over_a*((3.*sin_sfdm(pba,theta_sfdm_2)+omega_sfdm_2*(1.-cos_sfdm(pba,theta_sfdm_2)))*delta1_sfdm_2
                                      -omega_sfdm_2*sin_sfdm(pba,theta_sfdm_2)*delta_sfdm_2)
                                     -metric_continuity*(1.-cos_sfdm(pba,theta_sfdm_2)); //metric_continuity = h'/2;
      /*delta_1*/
     dy[pv->index_pt_delta1_sfdm_2] = -a_prime_over_a*((3.*cos_sfdm(pba,theta_sfdm_2)+(omega_sfdm_2-0.5*pba->sfdm_parameters_2[1]*exp(alpha_sfdm_2-y1_sfdm_2)/
                                      pow(1.-pba->sfdm_parameters_2[1]*exp(alpha_sfdm_2-2.*y1_sfdm_2)*(1.+cos_sfdm(pba,theta_sfdm_2)),0.5))*
                                      sin_sfdm(pba,theta_sfdm_2))*delta1_sfdm_2
                                      -(omega_sfdm_2-0.5*pba->sfdm_parameters_2[1]*exp(alpha_sfdm_2-y1_sfdm_2)/
                                       pow(1.-pba->sfdm_parameters_2[1]*exp(alpha_sfdm_2-2.*y1_sfdm_2)*(1.+cos_sfdm(pba, theta_sfdm_2)),0.5))*
                                      (1.+cos_sfdm(pba,theta_sfdm_2))*delta_sfdm_2)
                                      -metric_continuity*sin_sfdm(pba,theta_sfdm_2);
     }
    ...
    if (pba->has_scf == _TRUE_) {
      ...
      /** - ----> Klein Gordon equation* \delta_dotphi= a*a_prime_over_a/sqrt(8*_PI_*_G_)*sqrt(3/2)*exp(0.5*y[pba->index_bi_alpha_sfdm_1])*(delta1_sfdm_1*cos_sfdm(pba,0.5*theta_sfdm_1)+delta_sfdm_1*sin_sfdm(pba, 0.5*theta_sfdm_1)) */
      ...
      dy[pv->index_pt_phi_prime_scf] =  - 2.*a_prime_over_a*y[pv->index_pt_phi_prime_scf]
        ...
        -pba->int_sf*a_prime_over_a*a_prime_over_a/(8*_PI_*_G_)*6*exp(y[pba->index_bi_alpha_sfdm_1])*sin_sfdm(pba, 0.5*theta_sfdm_1)*((delta1_sfdm_1*cos_sfdm(pba,0.5*theta_sfdm_1)+delta_sfdm_1*sin_sfdm(pba, 0.5*theta_sfdm_1))*pvecback[pba->index_bg_phi_prime_scf]/a+sin_sfdm(pba, 0.5*theta_sfdm_1)*pv->index_pt_phi_prime_scf/a); //checked
