...
int input_read_parameters(
  ...
  double theta_ini_1, alpha_ini_1; //Auxiliar variables for cos potential
  double theta_sfdm_1, alpha_sfdm_1, y1_sfdm_1;
  double aosc_1, aosc3_1, b3_1; /*aguess1_1, aguess2_1; //Auxiliar variables for cos potential*/
  double theta0_1, theta1_1, theta2_1, theta3_1, alpha1_1, alpha2_1, alpha3_1; //Auxiliar variables for cos potential
  double masstohubble_ini_1, y1_aux_sfdm_1; /*Auxiliar variables for cosh*/
  //double aguess1,aguess2;
  /*Scalar Field Dark Matter 2*/
  double theta_ini_2, alpha_ini_2; //Auxiliar variables for cos potential
  double theta_sfdm_2, alpha_sfdm_2, y1_sfdm_2;
  double aosc_2, aosc3_2, b3_2; /*aguess1_2, aguess2_2; //Auxiliar variables for cos potential*/
  double theta0_2, theta1_2, theta2_2, theta3_2, alpha1_2, alpha2_2, alpha3_2; //Auxiliar variables for cos potential
  double masstohubble_ini_2, y1_aux_sfdm_2; /*Auxiliar variables for cosh*/
  /**/
  double scf_lambda;
  double fnu_factor;
  double * pointer1;
  ...
  /*--------------------------------------------------------*/
  /*Scalar Field Dark Matter 1. Read the current value of Omega_sfdm_1 from the .ini archive*/
  /*--------------------------------------------------------*/
  class_call(parser_read_double(pfc,"Omega_sfdm_1",&param1,&flag1,errmsg),
               errmsg,
               errmsg);
  class_call(parser_read_double(pfc,"omega_sfdm_1",&param2,&flag2,errmsg),
               errmsg,
               errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
               errmsg,
               "In input file, you can only enter one of Omega_sfdm_1 or omega_sfdm_1, choose one");
  if (flag1 == _TRUE_)
    pba->Omega0_sfdm_1 = param1;
  if (flag2 == _TRUE_)
    pba->Omega0_sfdm_1 = param2/pba->h/pba->h;

  Omega_tot += pba->Omega0_sfdm_1;

    /* Additional SFDM 1 parameters: */
    if (pba->Omega0_sfdm_1 != 0.){
        /** - Read parameters describing scalar field potential */
        class_call(parser_read_list_of_doubles(pfc,
                                               "sfdm_parameters_1",
                                               &(pba->sfdm_parameters_size_1),
                                               &(pba->sfdm_parameters_1),
                                               &flag1,
                                               errmsg),
                   errmsg,errmsg);
        class_read_int("sfdm_tuning_index_1",pba->sfdm_tuning_index_1);
        class_test(pba->sfdm_tuning_index_1 >= pba->sfdm_parameters_size_1,
                   errmsg,
                   "Tuning index sfdm_tuning_index_1 = %d is larger than the number of entries %d in sfdm_parameters_1. Check your .ini file.",pba->sfdm_tuning_index_1,pba->sfdm_parameters_size_1);
        /** - Assign shooting parameter */
        class_read_double("sfdm_shooting_parameter_1",pba->sfdm_parameters_1[pba->sfdm_tuning_index_1]);
        /*Initial conditions for sfdm_1*/
        //Cos and quadratic.
        if (pba->sfdm_parameters_1[1] >= 0.){
          theta_ini_1 = 0.4*15.64*pow(10.,pba->sfdm_parameters_1[0])/(pow(pba->Omega0_g+pba->Omega0_ur,0.5)*pba->H0);
          /*Newton-Raphson to solve the cubic correction for aosc when lambda > 0.*/
          aosc_1 = pow((0.5*_PI_/theta_ini_1)/pow(1.+pow(_PI_,2)/36.,0.5),0.5);
          b3_1 = 1.e-14*pba->sfdm_parameters_1[1]*pba->Omega0_sfdm_1/(72.*(pba->Omega0_g+pba->Omega0_ur));
          aosc3_1 = pow(aosc_cubic(aosc_1, b3_1),3.);
          /*Calcule pivot value of Omega_ini to calcule appropiate initial conditions.*/
          alpha_ini_1 = pba->sfdm_parameters_1[pba->sfdm_tuning_index_1] + log(pba->Omega0_sfdm_1*1.e-14/(pow(aosc_1,3.)*(pba->Omega0_g+pba->Omega0_ur)));
          /*Secant method to find the third correction for theta_sfdm*/
          if(pba->sfdm_parameters_1[1] > 0.){
            theta1_1 = theta_ini_1;
            alpha1_1 = alpha_ini_1;
            theta2_1 = (36.*_PI_/pow(pba->sfdm_parameters_1[1],2.))*1.e28*pow(2.5*theta1_1*(pba->Omega0_g+pba->Omega0_ur)/pba->Omega0_sfdm_1,2.);
            aosc_1 = pow((0.5*_PI_/theta2_1)/pow(1. + pow(_PI_,2)/36.,0.5),0.5);
            aosc3_1 = pow(aosc_cubic(aosc_1,b3_1),3.);
            alpha2_1 = log(pba->Omega0_sfdm_1*1e-14/(aosc3_1*(pba->Omega0_g+pba->Omega0_ur)));
            for(i=0; i<30; i++){
              theta3_1 = theta1_1 - (theta1_1 - theta2_1)*fmass(theta1_1, alpha1_1, theta_ini_1, pba->sfdm_parameters_1[1])/
                         (fmass(theta1_1, alpha1_1, theta_ini_1, pba->sfdm_parameters_1[1])- fmass(theta2_1, alpha2_1, theta_ini_1, pba->sfdm_parameters_1[1]));
              aosc_1 = pow((0.5*_PI_/theta3_1)/pow(1.+pow(_PI_,2)/36.,0.5),0.5);
              aosc3_1 = pow(aosc_cubic(aosc_1, b3_1),3.);
              alpha3_1 = pba->sfdm_parameters_1[pba->sfdm_tuning_index_1] + log(pba->Omega0_sfdm_1*1.e-14/(aosc3_1*(pba->Omega0_g+pba->Omega0_ur)));
              if(pow(pow(verify(theta3_1, alpha3_1, theta_ini_1, pba->sfdm_parameters_1[1]),2.),0.5)<1.e-6) break;
              theta1_1 = theta2_1;
              alpha1_1 = alpha2_1;
              theta2_1 = theta3_1;
              alpha2_1 = alpha3_1;
            }
            theta_ini_1 = theta3_1;
            alpha_ini_1 = alpha3_1;
          }
          pba->alpha_ini_sfdm_1 = alpha_ini_1;
          pba->theta_ini_sfdm_1 = theta_ini_1;
        }
        //Cosh
        else{
          theta_ini_1 = acos(-1./3.);
          alpha_ini_1 = log(-12./pba->sfdm_parameters_1[1]);
          masstohubble_ini_1 = 1.e-28*pow(pba->sfdm_parameters_1[1]/3.-4.,2.)*pow(pba->Omega0_sfdm_1/(pba->Omega0_g+pba->Omega0_ur),2.);
          //Now y1, instead of alpha will be adjusted using shooting parameter.
          y1_aux_sfdm_1 = log(2.*masstohubble_ini_1) + pba->sfdm_parameters_1[pba->sfdm_tuning_index_1];

          pba->alpha_ini_sfdm_1 = alpha_ini_1;
          pba->theta_ini_sfdm_1 = theta_ini_1;
          pba->y1_ini_sfdm_1 = y1_aux_sfdm_1;
        }
        ...
        if(flag1 == _TRUE_){
          if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
            ...
            if(pba->sfdm_parameters_1[1] >= 0.)
            pba->y1_ini_sfdm_1 = 5.*pba->theta_ini_sfdm_1;
            else /*cosh*/
            pba->y1_ini_sfdm_1 = y1_aux_sfdm_1;
            /*else
            pba->y1_ini_sfdm_1 = pow(8.,0.5);*/
          }
          else{
            pba->attractor_ic_sfdm_1 = _FALSE_;
            class_test(pba->sfdm_parameters_size_1<2,
                       errmsg,
                       "Since you are not using the attractor initial conditions,you must specify theta_1 and Omega_sfdm_1 as the last two entries in sfdm_parameters_1. See explanatory.ini for more details.");
                       pba->y1_ini_sfdm_1 = pba->sfdm_parameters_1[3];
          }
        }
    }
  /*--------------------------------------------------------*/
  /*Scalar Field Dark Matter 2 Read the current value of Omega_sfdm_2 from the .ini archive*/
  /*--------------------------------------------------------*/

  /** - Omega_0_sfdm_2 (SFCDM) */
  class_call(parser_read_double(pfc,"Omega_sfdm_2",&param1,&flag1,errmsg),
               errmsg,
               errmsg);
  class_call(parser_read_double(pfc,"omega_sfdm_2",&param2,&flag2,errmsg),
               errmsg,
               errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
               errmsg,
               "In input file, you can only enter one of Omega_sfdm_2 or omega_sfdm_2, choose one");
  if (flag1 == _TRUE_)
    pba->Omega0_sfdm_2 = param1;
  if (flag2 == _TRUE_)
    pba->Omega0_sfdm_2 = param2/pba->h/pba->h;

  Omega_tot += pba->Omega0_sfdm_2;
  /* Additional SFDM 2 parameters: */
  if (pba->Omega0_sfdm_2 != 0.){
      /** - Read parameters describing scalar field potential */
      class_call(parser_read_list_of_doubles(pfc,
                                             "sfdm_parameters_2",
                                             &(pba->sfdm_parameters_size_2),
                                             &(pba->sfdm_parameters_2),
                                             &flag1,
                                             errmsg),
                 errmsg,errmsg);
      class_read_int("sfdm_tuning_index_2",pba->sfdm_tuning_index_2);
      class_test(pba->sfdm_tuning_index_2 >= pba->sfdm_parameters_size_2,
                 errmsg,
                 "Tuning index sfdm_tuning_index_2 = %d is larger than the number of entries %d in sfdm_parameters_2. Check your .ini file.",pba->sfdm_tuning_index_2,pba->sfdm_parameters_size_2);
      /** - Assign shooting parameter */
      class_read_double("sfdm_shooting_parameter_2",pba->sfdm_parameters_2[pba->sfdm_tuning_index_2]);
      /*Initial conditions for sfdm_2*/
      if (pba->sfdm_parameters_2[1] >= 0.){
        theta_ini_2 = 0.4*15.64*pow(10.,pba->sfdm_parameters_2[0])/(pow(pba->Omega0_g+pba->Omega0_ur,0.5)*pba->H0);
        /*Newton-Raphson to solve the cubic correction for aosc when lambda > 0.*/
        aosc_2 = pow((0.5*_PI_/theta_ini_2)/pow(1.+pow(_PI_,2)/36.,0.5),0.5);
        b3_2 = 1.e-14*pba->sfdm_parameters_2[1]*pba->Omega0_sfdm_2/(72.*(pba->Omega0_g+pba->Omega0_ur));
        aosc3_2 = pow(aosc_cubic(aosc_2, b3_2),3.);
        /*Calcule pivot value of Omega_ini to calcule appropiate initial conditions.*/
        alpha_ini_2 = pba->sfdm_parameters_2[pba->sfdm_tuning_index_2] + log(pba->Omega0_sfdm_2*1.e-14/(pow(aosc_2,3.)*(pba->Omega0_g+pba->Omega0_ur)));
        /*Secant method to find the third correction for theta_sfdm*/
        if(pba->sfdm_parameters_2[1] > 0.){
          theta1_2 = theta_ini_2;
          alpha1_2 = alpha_ini_2;
          theta2_2 = (36.*_PI_/pow(pba->sfdm_parameters_2[1],2.))*1.e28*pow(2.5*theta1_2*(pba->Omega0_g+pba->Omega0_ur)/pba->Omega0_sfdm_2,2.);
          aosc_2 = pow((0.5*_PI_/theta2_2)/pow(1. + pow(_PI_,2)/36.,0.5),0.5);
          aosc3_2 = pow(aosc_cubic(aosc_2,b3_2),3.);
          alpha2_2 = log(pba->Omega0_sfdm_2*1e-14/(aosc3_2*(pba->Omega0_g+pba->Omega0_ur)));
          for(i=0; i<30; i++){
            theta3_2 = theta1_2 - (theta1_2 - theta2_2)*fmass(theta1_2, alpha1_2, theta_ini_2, pba->sfdm_parameters_2[1])/
                       (fmass(theta1_2, alpha1_2, theta_ini_2, pba->sfdm_parameters_2[1])- fmass(theta2_2, alpha2_2, theta_ini_2, pba->sfdm_parameters_2[1]));
            aosc_2 = pow((0.5*_PI_/theta3_2)/pow(1.+pow(_PI_,2)/36.,0.5),0.5);
            aosc3_2 = pow(aosc_cubic(aosc_2, b3_2),3.);
            alpha3_2 = pba->sfdm_parameters_2[pba->sfdm_tuning_index_2] + log(pba->Omega0_sfdm_2*1.e-14/(aosc3_2*(pba->Omega0_g+pba->Omega0_ur)));
            if(pow(pow(verify(theta3_2, alpha3_2, theta_ini_2, pba->sfdm_parameters_2[1]),2.),0.5)<1.e-6) break;
            theta1_2 = theta2_2;
            alpha1_2 = alpha2_2;
            theta2_2 = theta3_2;
            alpha2_2 = alpha3_2;
          }
          theta_ini_2 = theta3_2;
          alpha_ini_2 = alpha3_2;
        }
        pba->alpha_ini_sfdm_2 = alpha_ini_2;
        pba->theta_ini_sfdm_2 = theta_ini_2;
      }
      //Cosh
      else{
        theta_ini_2 = acos(-1./3.);
        alpha_ini_2 = log(-12./pba->sfdm_parameters_2[1]);
        masstohubble_ini_2 = 1.e-28*pow(pba->sfdm_parameters_2[1]/3.-4.,2.)*pow(pba->Omega0_sfdm_2/(pba->Omega0_g+pba->Omega0_ur),2.);
        //Now y1, instead of alpha will be adjusted using shooting parameter.
        y1_aux_sfdm_2 = log(2.*masstohubble_ini_2) + pba->sfdm_parameters_2[pba->sfdm_tuning_index_2];

        pba->alpha_ini_sfdm_2 = alpha_ini_2;
        pba->theta_ini_sfdm_2 = theta_ini_2;
        pba->y1_ini_sfdm_2 = y1_aux_sfdm_2;
      }
      class_call(parser_read_string(pfc,
                                    "attractor_ic_sfdm_2",
                                    &string1,
      ...
      if(flag1 == _TRUE_){
        if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
          ...
          if(pba->sfdm_parameters_2[1] >= 0.)
          pba->y1_ini_sfdm_2 = 5.*pba->theta_ini_sfdm_2;
          else //cosh
          pba->y1_ini_sfdm_2 = y1_aux_sfdm_2;
          /*else
          pba->y1_ini_sfdm_2 = pow(8.,0.5);*/
        }
        else{
          pba->attractor_ic_sfdm_2 = _FALSE_;
          class_test(pba->sfdm_parameters_size_2<2,
                     errmsg,
                     "Since you are not using the attractor initial conditions,you must specify theta_2 and Omega_sfdm_2 as the last two entries in sfdm_parameters_2. See explanatory.ini for more details.");
                     pba->y1_ini_sfdm_2 = pba->sfdm_parameters_2[3];
        }
      }
  }
...
int input_default_params(
  ...
  /*Scalar Field Dark Matter 1 defaults*/
  pba->Omega0_sfdm_1 = 0.; /* Scalar field dark matter defaults */
  pba->attractor_ic_sfdm_1 = _TRUE_;
  ...
  /*Scalar Field Dark Matter 2 defaults*/
  pba->Omega0_sfdm_2 = 0.; /* Scalar field dark matter defaults */
  pba->attractor_ic_sfdm_2 = _TRUE_;
...
int input_default_precision ( struct precision * ppr ) {
  ...
  ppr->a_ini_over_a_today_default = 1.e-30;
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    /*Scalar Field Dark Matter 1*/
    case Omega_sfdm_1:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_sfdm_1]/(ba.H0*ba.H0)
        -ba.Omega0_sfdm_1;
      break;
    /*Scalar Field Dark Matter 2*/
    case Omega_sfdm_2:
      output[i] = ba.background_table[(ba.bt_size-1)*ba.bg_size+ba.index_bg_rho_sfdm_2]/(ba.H0*ba.H0)
        -ba.Omega0_sfdm_2;
      break;
    /**   case omega_sfdm:
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_sfdm_1:
      ...
      xguess[index_guess] = ba.sfdm_parameters_1[ba.sfdm_tuning_index_1];
      dxdy[index_guess] = 1.;
      break;
    /*Scalar Field Dark Matter 2*/
    case Omega_sfdm_2:
      /* Default: take the passed value as xguess and set dxdy to 1. */
      xguess[index_guess] = ba.sfdm_parameters_2[ba.sfdm_tuning_index_2];
      dxdy[index_guess] = 1.;
      break;
    /*    case omega_sfdm:
  ...
  * - Version 3: use attractor solution */
      ...
      if (ba.scf_tuning_index == 0){
        /*xguess[index_guess] = sqrt(3.0/ba.Omega0_scf); MOD:this index_guess is too high for EDE and radiation*/ 
        xguess[index_guess] = ba.scf_parameters[ba.scf_tuning_index];
        /*dxdy[index_guess] = -0.5*sqrt(3.0)*pow(ba.Omega0_scf,-1.5);*/
        dxdy[index_guess] = -0.7;
      }
      else{
        /* Default: take the passed value as xguess and set dxdy to 1. */
        xguess[index_guess] = ba.scf_parameters[ba.scf_tuning_index];
...
double fmass(double theta1, double Omega1, double theta0, double lambda){
  return pow(theta1/theta0,2.) + 2.*lambda*exp(Omega1)/pow(5.*theta0,2.) -1.;
}

double verify(double theta_ini, double Omega_ini, double theta0, double lambda){
  return (pow(theta_ini/theta0, 2.) + 2.*lambda*exp(Omega_ini)/pow(5.*theta0,2.) -1.)/(pow(theta_ini/theta0,2.)+ 2.*lambda*exp(Omega_ini)/pow(5.*theta0,2.)+1.);
}
